...
const downloadRSA = require('./download.rsa');
...
class Env {
    ...
    async setup()
    {
        ...
        const rsaFileName = 'baker_rsa';
        const downloadDestFullPath = path.join(slimdir, rsaFileName);

        try {
            // Ensure baker keys are installed.
            const downloadResult = await downloadRSA(downloadDestFullPath, { fileName: rsaFileName});
            if (downloadResult) {
                await fs.chmod(downloadDestFullPath, '600', () => {});
                debug('successfully downloaded RSA Key and set proper access control.');
            }
        } catch(err) {
            const exists = await fs.existsSync(downloadDestFullPath);
            if (!exists) {
                error(err);
            } else {
                debug('An error occurred while trying to download RSA Key and/or configure proper access control.');
                debug('However, ignoring this error and proceeding anyways (RSA Key already exists at target location).');
            }
        }
