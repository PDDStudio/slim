...
class Env {
    ...
    async setup()
    {
        ...
        try {
            ...
            await fs.copyFileSync(path.join(scriptdir, 'scripts', 'keys', 'baker_rsa'), path.join(slimdir, 'baker_rsa'));
            await fs.chmod(path.join(slimdir, 'baker_rsa'), '600', () => {});
        } catch(error) {
            const exists = await fs.existsSync(path.join(slimdir, 'baker_rsa'));
            if (!exists) {
                console.error(error);
            }
        }
